# -*- coding: utf-8 -*-
"""
Created on Sat Oct 31 21:06:14 2020
...
def comparison():
    ...
    df4 = pd.read_csv("E:/Naaniz Internship/Comparison Algo/frendycleanc.csv")
    df5 = pd.read_csv("E:/Naaniz Internship/Comparison Algo/grofersclean.csv")
    df6 = pd.read_csv("E:/Naaniz Internship/Comparison Algo/dmartclean.csv")
    ...
    df5_price = df5.loc[:,'Price']
    df5_site = df5.loc[:, 'Website']
    df5_qty = df5.loc[:, 'Quantity']
    df5_unit = df5.loc[:, 'Unit']
    df5 = df5.loc[:, 'Item']
    
    df6_price = df6.loc[:,'Price']
    df6_site = df6.loc[:, 'Website']
    df6_qty = df6.loc[:, 'Quantity']
    df6_unit = df6.loc[:, 'Unit']
    df6 = df6.loc[:, 'Item']
    ...
    df3_price = df3_price.append(df5_price)
    df3_price = df3_price.append(df6_price)
    
    df3_site = df1_site.append(df2_site)
    df3_site = df3_site.append(df4_site)
    df3_site = df3_site.append(df5_site)
    df3_site = df3_site.append(df6_site)
    
    df3_qty = df1_qty.append(df2_qty)
    df3_qty = df3_qty.append(df4_qty)
    df3_qty = df3_qty.append(df5_qty)
    df3_qty = df3_qty.append(df6_qty)
    
    df3_units = df1_unit.append(df2_unit)
    df3_units = df3_units.append(df4_unit)
    df3_units = df3_units.append(df5_unit)
    df3_units = df3_units.append(df6_unit)
            ...
            for k in j.split():
                if i in k:
                    if i==k :
                        ...
                        #print(counter)
                        overall(counter)
        forward_data(final_brands, final_items, final_prices, final_sites, final_quantity, final_units)
    
    def forward_data(brand, items, prices, site, qty, units):
        ds_price=[]
        ds_brand=[]
        ds_item=[]
        ds_qty=[]
        ...
        for i in site:
            ...
            if i == 'DS':
                ...
                grofers_price.append(prices[counter])
                grofers_brand.append(brand[counter])
                grofers_item.append(items[counter])
                grofers_qty.append(qty[counter])
                grofers_unit.append(units[counter])
            elif i == 'DMart':
                dmart_price.append(prices[counter])
                dmart_brand.append(brand[counter])
                dmart_item.append(items[counter])
                dmart_qty.append(qty[counter])
                dmart_unit.append(units[counter])
            #print(counter)
        ...
        for i in frendy_item:
            ...
            if i in grofers_item:
                
                index = grofers_item.index(i)
                index_frendy = frendy_item.index(i)
                
                if frendy_brand[index_frendy] == grofers_brand[index] and i != prev_item and frendy_qty[index_frendy] == grofers_qty[index]:         
                    print('Found in Grofers: ' + i + 'at ' + str(index))
                    final_brands.append(frendy_brand[index_frendy])
                    final_items.append(i)
                    final_price_frendy.append(frendy_price[index_frendy])
                    final_price_ds.append('-')
                    final_price_jiomart.append('-')
                    final_price_grofers.append(grofers_price[index])
                    final_price_dmart.append('-')
                    final_qty.append(frendy_qty[index_frendy])
                    final_unit.append(frendy_unit[index_frendy])
                    prev_item = i
            ...
            if i in dmart_item:
                ...
                index = dmart_item.index(i)
                index_frendy = frendy_item.index(i)
                
                if frendy_brand[index_frendy] == dmart_brand[index] and i != prev_item and frendy_qty[index_frendy] == dmart_qty[index]:         
                    print('Found in DMart: ' + i + 'at ' + str(index))
                    final_brands.append(frendy_brand[index_frendy])
                    final_items.append(i)
                    final_price_frendy.append(frendy_price[index_frendy])
                    final_price_ds.append('-')
                    final_price_jiomart.append('-')
                    final_price_grofers.append('-')
                    final_price_dmart.append(dmart_price[index])
                    final_qty.append(frendy_qty[index_frendy])
                    final_unit.append(frendy_unit[index_frendy])
                    prev_item = i
        ...
        if len(final_items) == 0:
            ...
            #print(final_qty)
            #print(final_unit)
            col = pd.DataFrame(list(zip(final_brands, final_items, final_qty, final_unit, final_price_frendy, final_price_ds, final_price_jiomart, final_price_grofers, final_price_dmart)), columns =['Brand Name', 'Item Name', 'Quantity', 'Unit', 'Frendy Price', 'DealShare Price', 'JioMart Price', 'Grofers Price', 'DMart Price']) 
